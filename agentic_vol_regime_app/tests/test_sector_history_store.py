...
from agentic_vol_regime_app.data.ibkr_client import IBKRDailyBar, IBKRDailyHistoryRequest, IBKRDailyHistoryResult
...
class FakeDailyBarClient:
    ...
    def request_daily_bars(self, request: IBKRDailyHistoryRequest) -> IBKRDailyHistoryResult:
        ...
        if self.fail_on_request_number is not None and request_number == self.fail_on_request_number:
            raise RuntimeError("synthetic IBKR request failure")
        end_date = pd.Timestamp(request.end_date or max(self.history_by_symbol[request.symbol].keys()))
        start_date = end_date - pd.Timedelta(days=max(int(request.duration_calendar_days), 1) - 1)
        self.requests.append(
            {
                "symbol": request.symbol,
                "end_date": end_date.date(),
                "duration_calendar_days": int(request.duration_calendar_days),
                "estimated_start": start_date.date() if hasattr(start_date, "date") else start_date,
            }
        )
        actual_what = self.actual_what_to_show_by_symbol.get(request.symbol, request.preferred_what_to_show)
        bars = []
        for session_date, close in sorted(self.history_by_symbol.get(request.symbol, {}).items()):
        ...
        return IBKRDailyHistoryResult(bars=tuple(bars), actual_what_to_show=actual_what, warnings=())
...
def _build_store(tmp_path: Path, symbols: tuple[str, ...] = TEST_SYMBOLS) -> SectorPriceStore:
    parquet_path, metadata_path = _store_paths(tmp_path)
    return SectorPriceStore(parquet_path=parquet_path, metadata_path=metadata_path, symbols=symbols)
...
def _make_frame_from_histories(histories: dict[str, dict[date, float]], symbols: tuple[str, ...] = TEST_SYMBOLS) -> pd.DataFrame:
    all_dates = sorted({day for symbol_history in histories.values() for day in symbol_history.keys()})
    rows = []
...
def test_bootstrap_refuses_overwrite_without_force(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-02", 5, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories)
...
def test_second_update_on_already_current_store_makes_zero_requests(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-02", 6, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    ...
    assert all(payload.status == "already_current" for payload in result.symbols.values())


def test_update_requests_only_delta_plus_overlap(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 10, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories, overlap_trading_days=5)
    frame = _make_frame_from_histories({symbol: dict(list(values.items())[:9]) for symbol, values in histories.items()})
    _write_existing_store(store, frame)

    updater.update(target_end_date="2026-07-13")

    assert len(client.requests) == 3
    assert max(int(request["duration_calendar_days"]) for request in client.requests) <= 20


def test_different_per_symbol_watermarks_produce_different_request_ranges(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 10, base=150.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    ...
    estimated_starts = {request["symbol"]: request["estimated_start"] for request in client.requests}
    assert estimated_starts["XLE"] != estimated_starts["SPY"]


def test_missing_stored_history_for_one_symbol_does_not_force_all_symbols_to_refetch(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 10, base=120.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories)
    frame = _make_frame_from_histories(histories)
    frame["XLE"] = pd.NA
    _write_existing_store(store, frame)

    result = updater.update(target_end_date="2026-07-13", bootstrap_start_date="2026-06-30")
...
def test_overlapping_returned_dates_are_deduplicated(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2025-01-02", 280, base=180.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories, chunk_calendar_days=180)
...
def test_new_valid_overlapping_values_replace_stored_values(tmp_path: Path) -> None:
    ...
    existing = _make_frame_from_histories({symbol: dict(list(values.items())[:6]) for symbol, values in histories.items()})
    ...
    assert float(frame.loc[frame["date"] == pd.Timestamp(revised_day), "XLK"].iloc[0]) == 999.0


def test_revised_value_count_is_accurate(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 7, base=300.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
...
def test_missing_price_remains_missing_and_is_never_forward_filled(tmp_path: Path) -> None:
    missing_day = _business_days("2026-06-30", 8)[4]
    histories = {
        "XLK": _make_symbol_history("2026-06-30", 8, base=200.0, skip_dates={missing_day}),
...
def test_missing_bar_does_not_shift_dates_or_misalign_other_symbols(tmp_path: Path) -> None:
...
def test_offline_mode_makes_zero_ibkr_requests(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 5, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories)
    updater.bootstrap(start_date="2026-06-30", target_end_date="2026-07-06")
    client.requests.clear()

    frame = load_sector_price_history(parquet_path=store.parquet_path, metadata_path=store.metadata_path, symbols=TEST_SYMBOLS)
...
def test_offline_mode_succeeds_when_ibkr_is_unavailable(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 5, base=110.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories)
    updater.bootstrap(start_date="2026-06-30", target_end_date="2026-07-06")

    class BrokenClient:
        def request_daily_bars(self, _request):  # pragma: no cover - should never be called
...
def test_zero_negative_and_infinity_values_are_rejected_while_nan_is_preserved(tmp_path: Path) -> None:
    store = _build_store(tmp_path)
    valid_frame = pd.DataFrame([{"date": pd.Timestamp("2026-07-01"), "XLK": pd.NA, "XLE": 10.0, "SPY": 20.0}])
    assert store.validate_frame(valid_frame).row_count == 1

    for bad_value in (0.0, -1.0, float("inf")):
        frame = pd.DataFrame([{"date": pd.Timestamp("2026-07-01"), "XLK": bad_value, "XLE": 10.0, "SPY": 20.0}])
        with pytest.raises(ValueError):
            store.validate_frame(frame)


def test_failed_update_leaves_prior_authoritative_store_unchanged(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 7, base=200.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories)
    ...
    after = SectorPriceStore(parquet_path=store.parquet_path, metadata_path=store.metadata_path, symbols=TEST_SYMBOLS).load_offline()
...
def test_weekend_explicit_target_dates_normalize_consistently() -> None:
    assert resolve_target_completed_session(explicit_target_end_date="2026-07-12") == date(2026, 7, 10)
...
def test_actual_what_to_show_is_recorded_accurately(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 5, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories, actual_what_to_show_by_symbol={"XLK": "ADJUSTED_LAST", "XLE": "ADJUSTED_LAST", "SPY": "ADJUSTED_LAST"})
...
def test_trades_fallback_generates_warning_and_is_not_labeled_adjusted(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 5, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, _client = _make_updater(tmp_path, histories, actual_what_to_show_by_symbol={"XLE": "TRADES"})
...
def test_long_bootstrap_chunking_preserves_dates_and_deduplicates_overlaps(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2025-01-02", 320, base=100.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
...
def test_update_does_not_issue_full_history_request_regression(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 10, base=140.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories, overlap_trading_days=5)
    existing = _make_frame_from_histories({symbol: dict(list(values.items())[:9]) for symbol, values in histories.items()})
    _write_existing_store(store, existing)

    updater.update(target_end_date="2026-07-13", bootstrap_start_date="2020-01-01")

    assert client.requests
    assert max(int(request["duration_calendar_days"]) for request in client.requests) < 30


def test_repeating_same_update_is_idempotent(tmp_path: Path) -> None:
    histories = {symbol: _make_symbol_history("2026-06-30", 10, base=160.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories)
...
def test_recent_internal_gap_is_repaired_with_bounded_request(tmp_path: Path) -> None:
    ...
    histories = {symbol: _make_symbol_history("2026-06-30", 8, base=170.0 + index) for index, symbol in enumerate(TEST_SYMBOLS)}
    store, updater, client = _make_updater(tmp_path, histories, repair_horizon_trading_days=5)
